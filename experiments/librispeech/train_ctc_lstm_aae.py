...
from asr_vae.models.ctc_lstm_aae_model import make_ctc_lstm_aae_model_fn


def main(argv):
    asr_vae.trainer.main(argv, make_ctc_lstm_aae_model_fn)


if __name__ == '__main__':
    tf.logging.set_verbosity(tf.logging.INFO)
    tf.flags.DEFINE_string('model_dir', '../output/librispeech/lstm/ctc_aae/v7', 'Model directory')
    tf.flags.DEFINE_string('data_dir', '../dataset', 'Data directory')
    tf.flags.DEFINE_string('schedule', 'train_and_evaluate', 'Schedule')
    tf.flags.DEFINE_integer('batch_size', 16, 'Batch size')
    tf.flags.DEFINE_integer('max_steps', 10000, 'max_steps')
    tf.flags.DEFINE_integer('capacity', 4000, 'capacity')
    tf.flags.DEFINE_integer('min_after_dequeue', 2000, 'min_after_dequeue')
    tf.flags.DEFINE_integer('repeats', 10, 'repeats')
