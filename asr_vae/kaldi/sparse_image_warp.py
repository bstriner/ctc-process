...
from tensorflow.contrib.image.python.ops import dense_image_warp, interpolate_spline
from tensorflow.python.framework import constant_op, ops, tensor_shape
from tensorflow.python.ops import array_ops


def _get_grid_locations(image_height, image_width):
    """Wrapper for np.meshgrid."""
    y_range = tf.linspace(0., tf.cast(image_height-1, tf.float32), image_height)
    x_range = tf.linspace(0., tf.cast(image_width-1, tf.float32), image_width)
    y_grid, x_grid = tf.meshgrid(y_range, x_range, indexing='ij')
    return tf.stack((y_grid, x_grid), -1)


def _expand_to_minibatch(tf_array, batch_size):
    """Tile arbitrarily-sized np_array to include new batch dimension."""
    tiles = [batch_size] + [1] * tf_array.shape.ndims
    return tf.tile(tf.expand_dims(tf_array, 0), tiles)
