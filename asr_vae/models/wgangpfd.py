...
def wgangpfd(
    ...
    with tf.variable_scope(dis_scope, reuse=True):
        with tf.name_scope("InterpG"):
            gradint = tf.gradients(tf.reduce_sum(yint), [xint_padded])[0]  # (l,n,inputdim)
            gradnorm = tf.reduce_sum(tf.square(gradint), axis=(0, 2), keep_dims=True)  # (1,n,1)
            gradnorm = tf.sqrt(eps + gradnorm)
            step = step_size * gradint / gradnorm
            xstep_padded = xint_padded + step
            xstep_padded = tf.stop_gradient(xstep_padded)
            xstep_packed = tf.gather_nd(
                params=xstep_padded,
                indices=pack_idx
            )
            ystep = discriminator_fn(xstep_packed)

    with tf.name_scope("GP"):
        yd = tf.abs(ystep-yint)
        xd = step_size
        approxgrad = yd/xd
        penalty = tf.reduce_mean(tf.square(1-approxgrad))
